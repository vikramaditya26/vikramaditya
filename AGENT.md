...
### PHASE 2: Analytics & SEO Basics (Day 3) ✅ COMPLETED
...
### How analytics & SEO works (for future agents):
1. **Microsoft Clarity:** Snippet in `script.js` — replace `CLARITY_PROJECT_ID` with real ID. Free heatmaps + session recordings.
2. **JSON-LD:** Injected via `script.js` `injectStructuredData()`. WebSite schema on all pages, Person schema on homepage. Add more schemas per section as needed.
3. **OG tags:** In each page's `<head>`. Each page has unique `og:title`, `og:description`, `og:url`. No og:image yet — add when we have section-specific images.
...
### How the kitchen & diet tools work (for future agents):
1. **Food database:** `assets/data/foods.json` stores approximate macros, costs, diet compatibility, meal types, and equipment support for an initial seed set of foods. The roadmap target is still to expand this toward 200+ foods.
2. **Kitchen page:** `/kitchen/index.html` is mostly static editorial content plus an equipment filter. `initKitchenFilter()` in `script.js` shows or hides `.recipe-card` elements based on the selected setup (kettle, hot plate, full kitchen).
3. **Diet planner:** `/tools/diet-planner/index.html` uses `loadFoodsData()`, `computeNutritionTargets()`, and `buildDietPlan()` in `script.js`. The planner calculates BMR/TDEE using Mifflin-St Jeor, applies a goal adjustment, and then assembles a daily plan from `DIET_PLAN_TEMPLATES` plus a few food add-ons if protein or calories are still low.
4. **Macro calculator:** `/tools/macro-calculator/index.html` uses the same nutrition target helper but skips meal generation. This keeps the macro logic centralized instead of duplicating formulas.
5. **Equipment logic:** `supportsEquipment()` uses a simple level map (`none` < `kettle` < `hot-plate` < `full-kitchen`) so both recipes and plan templates can degrade gracefully when someone has limited gear.
6. **Dynamic affiliate usage:** The diet planner renders supplement cards into the result panel after form submission, then calls `setDefaultBuyButtonLabels(scope)` and `loadAffiliateLinks(scope)` so the affiliate buttons hydrate even though the DOM was inserted later.

### How the skincare & style sections work (for future agents):
1. **Shared product data:** `assets/data/products.json` now stores both skincare product recommendations and style capsule essentials. This keeps product metadata centralized instead of scattering it across multiple pages.
2. **Skincare page:** `/skincare/index.html` is partly static editorial content plus three dynamic areas: current routine blocks, category/tier product grids, and the skin-type guide result. `initSkincareAndStyle()` in `script.js` handles all three.
3. **Skin-type guide:** The current form is intentionally simple. It scores three answers across `oily`, `dry`, `combination`, and `normal`, then renders a practical recommendation path. It is a guide, not a medical diagnosis.
4. **Style page:** `/style/index.html` uses the same shared product loader to render capsule wardrobe essentials into `#style-capsule-grid`. The rest of the page is static editorial guidance for outfit formulas, fit, color, and seasonal dressing.
